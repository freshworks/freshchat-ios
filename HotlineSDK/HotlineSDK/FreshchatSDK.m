...
-(void)updateConfig:(FreshchatConfig *)config andRegisterUser:(void(^)(NSError *error))completion{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if (config) {
            ...
            [store setBoolValue:config.hybridExperienceEnabled forKey:HOTLINE_DEFAULTS_HYBRID_EXPERIENCE_ENABLED];
...
-(void)processClearUserData:(void (^)())completion init:(BOOL)doInit andOldUser:(NSDictionary*) previousUser{
    ...
    config.hybridExperienceEnabled = [store boolValueForKey:HOTLINE_DEFAULTS_HYBRID_EXPERIENCE_ENABLED];
