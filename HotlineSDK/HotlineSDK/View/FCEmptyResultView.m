...
@implementation FCEmptyResultView
...
-(id)initWithImage:(UIImage *)image withType:(enum SupportType)solType andText:(NSString *)text {
    ...
    if (self) {
        ...
        [FCAutolayoutHelper centerX:self.emptyResultLabel onView:self];
        [FCAutolayoutHelper centerX:self.emptyResultImage onView:self];
        [FCAutolayoutHelper centerY:self.emptyResultImage onView:self M:0.5 C:0.0];
    }
    return self;
}
