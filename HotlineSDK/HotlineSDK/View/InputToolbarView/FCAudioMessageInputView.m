...
@interface FCAudioMessageInputView ()
...
@implementation FCAudioMessageInputView
...
-(id)initWithDelegate:(id <FDAudioInputDelegate>)delegate{
    ...
    if (self) {
        ...
        [self.dismissButton setImage:[self.theme getImageValueWithKey:IMAGE_AUDIO_TOOLBAR_CANCEL] forState:UIControlStateNormal];
        ...
        [self.sendButton setImage:[self.theme getImageValueWithKey:IMAGE_SEND_ICON] forState:UIControlStateNormal];
...
-(void)layoutSubviews{
    ...
    [FCAutolayoutHelper center:self.recordingLabel onView:self];
...
- (void)timerTick:(NSTimer *)timer{
    self.timeLabel.text= [NSString stringWithFormat:@"%02d:%02d",(int)[FCMessageHelper getTimeElapsedSinceStartOfRecording]/(int)60,(int)[FCMessageHelper getTimeElapsedSinceStartOfRecording]%(int)60];
}
