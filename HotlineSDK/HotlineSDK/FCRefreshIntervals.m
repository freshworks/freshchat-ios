...
-(long) getDefaultValue:(enum FCRefreshIntervalType) type {
    switch (type) {
        case responseTimeExpectationsFetchInterval:
            if ([FCUserDefaults getObjectForKey:CONFIG_RC_RESPONSE_TIME_EXPECTATION_FETCH_INTERVAL] != nil ) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_RESPONSE_TIME_EXPECTATION_FETCH_INTERVAL];
            }
            return 5 * ONE_MINUTE_IN_MS;
            break;
        case remoteConfigFetchInterval:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_API_FETCH_INTERVAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_API_FETCH_INTERVAL];
            }
            return ONE_DAY_IN_MS;
            break;
        case activeConvMinFetchInterval:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_ACTIVE_CONV_MIN_FETCH_INTERVAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_ACTIVE_CONV_MIN_FETCH_INTERVAL];
            }
            return 20 * ONE_SECONDS_IN_MS;
            break;
        case activeConvMaxFetchInterval:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_ACTIVE_CONV_MAX_FETCH_INTERVAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_ACTIVE_CONV_MAX_FETCH_INTERVAL];
            }
            return 60 * ONE_SECONDS_IN_MS;
            break;
        case msgFetchIntervalNormal:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_MSG_FETCH_INTERVAL_NORMAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_MSG_FETCH_INTERVAL_NORMAL];
            }
            return 30 * ONE_SECONDS_IN_MS;
            break;
        case msgFetchIntervalLaidback:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_MSG_FETCH_INTERVAL_LAIDBACK] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_MSG_FETCH_INTERVAL_LAIDBACK];
            }
            return 60 * ONE_SECONDS_IN_MS;
            break;
        case faqFetchIntervalNormal:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_FAQ_FETCH_INTERVAL_NORMAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_FAQ_FETCH_INTERVAL_NORMAL];
            }
            return 5 * ONE_MINUTE_IN_MS;
            break;
        case faqFetchIntervalLaidback:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_FAQ_FETCH_INTERVAL_LAIDBACK] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_FAQ_FETCH_INTERVAL_LAIDBACK];
            }
            return 7 * ONE_DAY_IN_MS;
            break;
        case channelsFetchIntervalNormal:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_NORMAL] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_NORMAL];
            }
            return 5 * ONE_MINUTE_IN_MS;
            break;
        case channelsFetchIntervalLaidback:
            if([FCUserDefaults getObjectForKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_LAIDBACK] != nil) {
                return [FCUserDefaults getLongForKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_LAIDBACK];
            }
            return 7 * ONE_DAY_IN_MS;
            break;
    }
...
-(void) updateValue:(long)value type:(enum FCRefreshIntervalType) type {
    switch (type) {
        case responseTimeExpectationsFetchInterval:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_RESPONSE_TIME_EXPECTATION_FETCH_INTERVAL];
            self.responseTimeExpectationsFetchInterval = value;
            break;
        case remoteConfigFetchInterval:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_API_FETCH_INTERVAL];
            self.remoteConfigFetchInterval = value;
            break;
        case activeConvMinFetchInterval:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_ACTIVE_CONV_MIN_FETCH_INTERVAL];
            self.activeConvMinFetchInterval = value;
            break;
        case activeConvMaxFetchInterval:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_ACTIVE_CONV_MAX_FETCH_INTERVAL];
            self.activeConvMaxFetchInterval = value;
            break;
        case msgFetchIntervalNormal:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_MSG_FETCH_INTERVAL_NORMAL];
            self.msgFetchIntervalNormal = value;
            break;
        case msgFetchIntervalLaidback:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_MSG_FETCH_INTERVAL_LAIDBACK];
            self.msgFetchIntervalLaidback = value;
            break;
        case faqFetchIntervalNormal:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_FAQ_FETCH_INTERVAL_NORMAL];
            self.faqFetchIntervalNormal = value;
            break;
        case faqFetchIntervalLaidback:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_FAQ_FETCH_INTERVAL_LAIDBACK];
            self.faqFetchIntervalLaidback = value;
            break;
        case channelsFetchIntervalNormal:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_NORMAL];
            self.channelsFetchIntervalNormal = value;
            break;
        case channelsFetchIntervalLaidback:
            [FCUserDefaults setLong:value forKey:CONFIG_RC_CHANNELS_FETCH_INTERVAL_LAIDBACK];
