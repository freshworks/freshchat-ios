...
#import "FDSTTweetLabel.h"
#import "FDSTTweetTextStorage.h"
#import "FDTheme.h"

#define STURLRegex @"(?i)\\b((?:[a-z][\\w-]+:(?:/{1,3}|[a-z0-9%])|www\\d{0,3}[.]|[a-z0-9.\\-]+[.][a-z]{2,4}/)(?:[^\\s()<>]+|\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\))+(?:\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\)|[^\\s`!()\\[\\]{};:'\".,<>?«»“”‘’]))"

@interface FDSTTweetLabel () <UITextViewDelegate>

@property (strong) FDSTTweetTextStorage *textStorage;
...
- (void)setupLabel:(UIColor *)textColor;
...
- (void)setupLabel:(UIColor *)textColor {
    ...
    _attributesText = @{NSForegroundColorAttributeName: textColor, NSFontAttributeName: [UIFont fontWithName:[self.theme chatBubbleFontName] size:[self.theme chatBubbleFontSize]]};
    _attributesLink = @{NSForegroundColorAttributeName: [self.theme hyperlinkColor], NSFontAttributeName: [UIFont fontWithName:[self.theme chatBubbleFontName] size:[self.theme chatBubbleFontSize]], NSUnderlineStyleAttributeName: @(NSUnderlineStyleSingle)};
...
- (void)determineHotWords {
    ...
    _textStorage = [[FDSTTweetTextStorage alloc] init];
