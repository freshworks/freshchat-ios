...
+(NSString *)itemCreatedDurationSinceDate:(NSDate*)date{
    ...
    if (intervalInSeconds < 10) {
        return FDLocalizedString(@"Just now");
    }else if(intervalInSeconds < FEW_SECONDS){
        return FDLocalizedString(@"few seconds ago");
    }else if (interval < FEW_MINUTES) {
        return FDLocalizedString(@"few minutes ago");
    }else if(interval >= FEW_MINUTES && interval < ONE_HOUR){
        return [NSString stringWithFormat:FDLocalizedString(@"n minutes ago"),(long)interval];
    }else if(interval >= ONE_HOUR && interval < ONE_DAY){
        NSInteger hours = (int)interval/ONE_HOUR;
        if (hours == 1) { return FDLocalizedString(@"an hour ago"); }
        return [NSString stringWithFormat:FDLocalizedString(@"n hours ago"),(long)hours];
    }else if(interval >=ONE_DAY && interval< ONE_MONTH){
        NSInteger days = (int)interval/ONE_DAY;
        if (days == 1) { return FDLocalizedString(@"a day ago"); }
        return [NSString stringWithFormat:FDLocalizedString(@"n days ago"),(long)days];
    }else{
        return FDLocalizedString(@"more than a month ago");
    }
}
...
+(NSString *)solutionsLastUpdatedWebFriendlyTime{
    ...
    NSDate *lastUpdatedTime = [secureStore objectForKey:MOBIHELP_DEFAULTS_SOLUTIONS_LAST_UPDATED_TIME_V2];
